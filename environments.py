...
class PredictionEnvironment():
    ...
    def getIdentifier(self):
        modelAggId = self.model_parameters
...
#                 modelAggId = "_radonPoint"
    ...
    def process_example(self, example):
        if isinstance(self.modelClass, dnn.model_higgs.DNNModel):
...
#         print predictionScore, loss
        self.total_regression_error += loss
        true_label = y_val[0]
        generate(Event(PREDICTION, self.parameters, self.current_round, (true_label, predictionScore, loss, idx), node.model.getModelIdentifier()))#, loss,self.current_round)))
        self.total_error += loss
        self.numberOfExamplesProcessed += len(x_val)
        
        if self.numberOfExamplesProcessed % 10000 == 0:
            print true_label, predictionScore, loss
