...
class SmoothlyDriftingDisjunction(SyntheticDataGenerator):
    def __init__(self, number_of_items, change_prob, examples_per_round=1):
        self.item_inclusion_prob = sqrt(1 - pow(0.5, 1 / float(number_of_items)))
        record_generator = UniformRecordGenerator(number_of_items, self.item_inclusion_prob)
...
class RapidlyDriftingLinearThreshold(SyntheticDataGenerator):
    def __init__(self, number_of_items, change_prob, examples_per_round=1):
        self.item_inclusion_prob = sqrt(1 - pow(0.5, 1 / float(number_of_items)))
        record_generator = UniformRecordGenerator(number_of_items, self.item_inclusion_prob)
...
class DriftingFixedTermCNF(SyntheticDataGenerator):
...
def main():
    inputStream = RapidlyDriftingDisjunction(20, 0.05)
    posSeen = 0
