...
def generate(experiment_root_folder, show_drift_times, show_sync_times, loss_function=ZeroOneClassificationLoss()):
    print "Plotting Accuracy. Loss function: " + loss_function.name
    plot_chart(experiment_root_folder, show_drift_times, show_sync_times, loss_function)
    plt.xlabel("Time")
    plt.ylabel("Accuracy")
    save_chart(experiment_root_folder + "charts/accuracy_" + loss_function.shortname + "_time." + FILE_FORMAT)
...
def write_metric(experiment_root_folder, metric):
    file_name = experiment_root_folder + "/charts/accuracy.txt"
...
#                 correct += 1
        err = loss_function(float(parts[1]), float(parts[2]))
        if err < 0.125:
            correct += 1
        total += 1
